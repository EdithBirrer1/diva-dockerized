...
then
  ...
  then
    SIZE_NETWORK=${SIZE_NETWORK} docker compose -f ./genesis-i2p.yml down --volumes
  fi

  cp "${PROJECT_PATH}"/build/genesis-i2p.yml ./genesis-i2p.yml
  SIZE_NETWORK=${SIZE_NETWORK} docker compose -f ./genesis-i2p.yml pull
  SIZE_NETWORK=${SIZE_NETWORK} docker compose -f ./genesis-i2p.yml up -d
